...
def main(options):
    ...
    with open(output_path, 'w+') as out:
        ...
        writer.writerow(["audio_hidden", "video_hidden", 'audio_dropout', 'video_dropout', 
    ...
    for i in range(total_settings):
        ...
        current_setting = (ahid, vhid,  adr, vdr,  factor_lr, lr, r, batch_sz, decay)
        if current_setting in seen_settings:
            continue
        else:
            seen_settings.add(current_setting)

        model = LMF(input_dims, (ahid, vhid),  (adr, vdr,  0.5), output_dim, r)
        if options['cuda']:
            model = model.cuda()
        ...
        if complete:
            ...
            for batch in test_iterator:
                ...
                y = Variable(batch[-1].view(-1, output_dim).float().type(DTYPE), requires_grad=False)
                output_test = best_model(x_a, x_v, )
            ...
            results = [ahid, vhid, adr, vdr,  factor_lr, lr, r, batch_sz, decay, min_valid_loss.cpu().data.numpy()]
