...
def main(options):
    ...
    train_set, valid_set, test_set, input_dims = load_iemocap(data_path, str.encode(emotion))
    ...
    if not os.path.isfile(output_path):
        with open(output_path, 'w+') as out:
            ...
            writer.writerow(
                ["video_hidden", 'text_hidden', 'video_dropout', 'text_dropout', 'factor_learning_rate', 'learning_rate', 'rank', 'batch_size', 'weight_decay','Best Validation CrossEntropyLoss', 'Test CrossEntropyLoss', 'Test F1-score', 'Test Accuracy Score'])
    ...
    for i in range(total_settings):
        ...
        current_setting = ( vhid, thid,  vdr, tdr, factor_lr, lr, r, batch_sz, decay)
        if current_setting in seen_settings:
            continue
        else:
            seen_settings.add(current_setting)

        model = LMF(input_dims, ( vhid, thid), thid_2, ( vdr, tdr, 0.5), output_dim, r)
        if options['cuda']:
            model = model.cuda()
        ...
        for e in range(epochs):
            ...
            for batch in train_iterator:
                ...
                x_v = Variable(x[0].float().type(DTYPE), requires_grad=False)
                x_t = Variable(x[1].float().type(DTYPE), requires_grad=False)
        ...
        if complete:
            ...
            with open(output_path, 'a+') as out:
                writer = csv.writer(out)
                writer.writerow([ vhid, thid,  vdr, tdr, factor_lr, lr, r, batch_sz, decay,
