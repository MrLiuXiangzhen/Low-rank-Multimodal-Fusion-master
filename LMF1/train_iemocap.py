...
def main(options):
    ...
    if not os.path.isfile(output_path):
        with open(output_path, 'w+') as out:
            ...
            writer.writerow(
                ["audio_hidden",  'text_hidden', 'audio_dropout',  'text_dropout',
    ...
    for i in range(total_settings):
        ...
        current_setting = (ahid,  thid, adr,  tdr, factor_lr, lr, r, batch_sz, decay)
        if current_setting in seen_settings:
            continue
        else:
            seen_settings.add(current_setting)

        model = LMF(input_dims, (ahid,  thid), thid_2, (adr,  tdr, 0.5), output_dim, r)
        if options['cuda']:
            model = model.cuda()
        ...
        if complete:
            ...
            with open(output_path, 'a+') as out:
                writer = csv.writer(out)
                writer.writerow([ahid,  thid, adr,  tdr, factor_lr, lr, r, batch_sz, decay,
