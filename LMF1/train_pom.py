...
def main(options):
    ...
    with open(output_path, 'w+') as out:
        ...
        writer.writerow([ "video_hidden", 'text_hidden', 'audio_dropout', 'video_dropout', 'text_dropout',
    ...
    for i in range(total_settings):
        ...
        current_setting = (ahid,  thid, vdr, tdr, factor_lr, lr, r, batch_sz, decay)
        if current_setting in seen_settings:
            continue
        else:
            seen_settings.add(current_setting)

        model = LMF(input_dims, (ahid,  thid), thid_2, ( vdr, tdr, 0.5), output_dim, r)
        if options['cuda']:
            model = model.cuda()
        ...
        if complete:
            ...
            for batch in test_iterator:
                ...
                x_t = Variable(x[1].float().type(DTYPE), requires_grad=False)
                y = Variable(batch[-1].view(-1, output_dim).float().type(DTYPE), requires_grad=False)
                output_test = best_model( x_v, x_t)
            ...
            results = [ahid,  thid,  vdr, tdr, factor_lr, lr, r, batch_sz, decay, min_valid_loss.cpu().data.numpy()]
